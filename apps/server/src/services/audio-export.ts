...
import { TMP_DIR, EXPORT_DIR } from '../config.js';
...
export async function exportChapterAudio(
  ...
  // 临时目录（DATA_DIR/export-tmp/<chapterId>）
  const tmpDir = join(TMP_DIR, chapterId);
  ...
  try {
    ...
    const outPath = join(EXPORT_DIR, `${chapterId}.${format}`);
...
export async function listExports(): Promise<string[]> {
  try {
    return await readdir(EXPORT_DIR);
  } catch {
    return [];
  }
}
...
/**
 * 清理某章节的导出音频（删除章节/书籍时调用，避免孤儿文件堆积）
 * 文件名形如 <chapterId>.mp3 / <chapterId>.wav
 */
export async function cleanupChapterExports(chapterId: string): Promise<void> {
  try {
    ...
    await Promise.all(
      files
        ...
        .map((f) => rm(join(EXPORT_DIR, f), { force: true }).catch(() => {})),
    );
  } catch {
    // 目录不存在时无需处理
  }

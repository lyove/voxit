...
export default function App() {
  ...
  const handleLoadVoices = async () => {
    ...
    const { provider, apiKey, workspaceId } = currentProject.providerConfig;
    if (!apiKey || !workspaceId) {
      message.warning('请在设置中填写 API Key 和 Workspace ID');
      return;
    }
    await loadVoices(provider, apiKey, workspaceId);
  };
  ...
  const handleExportChapter = async () => {
    ...
    try {
      ...
      if (fileName) {
        api.downloadExport(fileName, `${activeChapter.title}.mp3`);
  ...
  return (
    <Layout className="vx-app">
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleCreateProject}>
          ...
          <Form.Item noStyle shouldUpdate={(prev, cur) => prev.provider !== cur.provider}>
            {({ getFieldValue }) => {
              const provider = getFieldValue('provider') as VxProvider;
              const isDoubao = provider === VxProvider.DOUBAO;
              return (
                <>
                  <Form.Item name="apiKey" label={isDoubao ? 'Access Token' : 'API Key'} rules={[{ required: true }]}>
                    <Input.Password placeholder={isDoubao ? '豆包 access_token' : 'sk-xxx（阿里云北京地域）'} />
                  </Form.Item>
                  <Form.Item name="workspaceId" label={isDoubao ? 'App ID' : 'Workspace ID'} rules={[{ required: true }]}>
                    <Input placeholder={isDoubao ? '火山引擎 appid' : '百炼业务空间 ID'} />
                  </Form.Item>
                </>
              );
            }}
      ...
      >
        {currentProject ? (
          <VxVoicePanel
            provider={currentProject.providerConfig.provider}
            apiKey={currentProject.providerConfig.apiKey ?? ''}
            workspaceId={currentProject.providerConfig.workspaceId ?? ''}
          />
        ) : (
  ...
  return (
    <Form
      ...
      initialValues={{
        ...
        apiKey: book.providerConfig.apiKey ?? '',
        workspaceId: book.providerConfig.workspaceId ?? '',
      }}
      onFinish={onSubmit}
    >
      <Form.Item name="name" label="书籍名称" rules={[{ required: true }]}>

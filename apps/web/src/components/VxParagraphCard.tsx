...
export function VxParagraphCard({ paragraph, voices, index, capabilities }: Props) {
  ...
  const [previewUrl, setPreviewUrl] = useState<string | undefined>(paragraph.audioUrl);

  // 当 store 中 paragraph.audioUrl 更新时（如批量合成后刷新），同步到本地播放器
  useEffect(() => {
    setPreviewUrl(paragraph.audioUrl);
  }, [paragraph.audioUrl]);
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      if (res.audioUrl) setPreviewUrl(res.audioUrl);
      else if (res.audioData) setPreviewUrl(`data:audio/wav;base64,${res.audioData}`);
  ...
  const handleSynthesize = async () => {
    ...
    try {
      ...
      if (updated.audioUrl) setPreviewUrl(updated.audioUrl);

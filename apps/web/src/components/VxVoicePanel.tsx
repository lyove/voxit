/**
 * 发音人试听台 —— 列出发音人，点击试听固定示例句
 * 凭证由服务器 .env 统一提供，前端不接触 Key
 */
import { useEffect, useRef, useState } from 'react';
import { Card, List, Button, Tag, Space, message, Spin, Input } from 'antd';
import { PauseCircleOutlined, PlayCircleOutlined } from '@ant-design/icons';
import type { VxProvider, VxVoice } from '@voxit/core';
import { fetchVoices, http, extractError, normalizeAudioUrl } from '../api.js';

interface Props {
  provider: VxProvider;
}

export function VxVoicePanel({ provider }: Props) {
  const [voices, setVoices] = useState<VxVoice[]>([]);
  const [loading, setLoading] = useState(false);
  const [previewingId, setPreviewingId] = useState<string | null>(null);
  const [playingId, setPlayingId] = useState<string | null>(null);
  const [sampleText, setSampleText] = useState('我家的后面有一个很大的花园，那里有我童年所有的回忆。');
  const audioRef = useRef<HTMLAudioElement | null>(null);

  // 切换页面/卸载时终止试听播放
  useEffect(() => {
    return () => {
      audioRef.current?.pause();
    };
  }, []);

  const load = async () => {
    setLoading(true);
    try {
      const v = await fetchVoices(provider);
      setVoices(v);
    } catch (e) {
      message.error('加载发音人失败：' + (e as Error).message);
    } finally {
      setLoading(false);
    }
  };

  const previewVoice = async (voice: VxVoice) => {
    // 正在播放当前音频则暂停
    if (playingId === voice.id) {
      audioRef.current?.pause();
      return;
    }
    // 切到新音频前先停掉旧的
    audioRef.current?.pause();

    setPreviewingId(voice.id);
    try {
      const res = await http.post(`/providers/${provider}/preview`, {
        text: voice.sampleText ?? sampleText,
        voiceId: voice.id,
        format: 'wav',
        sampleRate: 24000,
      });
      const url = res.data.audioUrl
        ? normalizeAudioUrl(res.data.audioUrl)
        : res.data.audioData
          ? `data:audio/wav;base64,${res.data.audioData}`
          : undefined;
      if (url) {
        const audio = new Audio(url);
        audio.onended = () => setPlayingId(null);
        audio.onpause = () => setPlayingId(null);
        audioRef.current = audio;
        await audio.play();
        setPlayingId(voice.id);
      }
    } catch (e) {
      message.error(`试听 ${voice.name} 失败：` + extractError(e));
    } finally {
      setPreviewingId(null);
    }
  };

  return (
    <Card title="发音人试听台" size="small" style={{ marginBottom: 16 }}>
      <Space style={{ marginBottom: 12 }} wrap>
        <Button type="primary" onClick={load} loading={loading}>加载发音人</Button>
        <Input
          style={{ width: 360 }}
          value={sampleText}
          onChange={(e) => setSampleText(e.target.value)}
          placeholder="试听示例句"
        />
      </Space>
      <Spin spinning={loading}>
        <List
          size="small"
          dataSource={voices}
          renderItem={(v) => (
            <List.Item
              actions={[
                <Button
                  size="small"
                  icon={playingId === v.id ? <PauseCircleOutlined /> : <PlayCircleOutlined />}
                  loading={previewingId === v.id}
                  onClick={() => previewVoice(v)}
                >
                  {playingId === v.id ? '暂停' : '试听'}
                </Button>,
              ]}
            >
              <Space>
                <strong>{v.name}</strong>
                {v.gender && <Tag>{v.gender === 'male' ? '男' : v.gender === 'female' ? '女' : '中'}</Tag>}
                <span style={{ color: '#999' }}>{v.description}</span>
              </Space>
            </List.Item>
          )}
        />
      </Spin>
    </Card>
  );
}

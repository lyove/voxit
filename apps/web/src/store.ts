...
interface VxStore {
  ...
  loadVoices: (provider: string, apiKey: string, workspaceId: string) => Promise<void>;
...
export const useStore = create<VxStore>((set, get) => ({
  ...
  loadVoices: async (provider, apiKey, workspaceId) => {
    const voices = await api.fetchVoices(provider as any, apiKey, workspaceId);
    set({ voices });
  },
